...
class product_model extends db_connection
{
    ...
    /*Insert Product*/
    public function add_product_mdl($cat_id, $brand_id, $product_title, $product_price, $product_desc, $newImageName, $product_keywords, $product_qnty, $product_doe, $product_discount, $equipImageName) {
        $ndb = new db_connection();	
        $admin_id = $_SESSION['role'];
        $sql = "INSERT INTO `equip_hiring` (`product_cat`, `product_brand`, `product_title`, `product_price`, `product_desc`, `product_image`, `product_keywords`, `product_qnty` , `product_doe`, `product_discount`, `admin_id`, `equip_doc`) VALUES ('$cat_id', '$brand_id','$product_title', '$product_price', '$product_desc', '$newImageName', '$product_keywords', '$product_qnty', '$product_doe', '$product_discount', '$admin_id', '$equipImageName')";
        return $this->db_query($sql);
    }

    /* fetch all products */
    public function get_all_products_mdl(){
        $sql = "SELECT * FROM `equip_hiring` order by rand() LIMIT 0,20";
        return $this->db_fetch_all($sql);
    }

    public function get_products_by_price_mdl($minprice, $maxprice)
	{
		$sql = "SELECT * FROM `equip_hiring` WHERE `product_price` BETWEEN '$minprice' AND '$maxprice'";
		return $this->db_fetch_all($sql);
	}
    
    public function get_product_by_id_mdl($product_id){
        $sql = "SELECT * FROM `equip_hiring` WHERE `product_id` = '$product_id'";
    ...
    function  products_receipt_mdl($order_id){
        ...
        JOIN `equip_hiring` ON orderdetails.product_id = equip_hiring.product_id
        JOIN `categories` ON equip_hiring.product_cat = categories.cat_id
